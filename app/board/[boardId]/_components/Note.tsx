...
export default function Note({ layer, onPointerDown, id, selectionColor}: NoteProps) {
    ...
    return (
        <foreignObject x={x} y={y} width={width} height={height} onPointerDown={(e) => onPointerDown(e, id)}
            ...
            <ContentEditable html={value || "Text"} onChange={handleContentChange}
                ...
                style={{ fontSize: calculateFontSize(width, height), color: fill ? getContrastingTextColor(fill) : "#000"}}></ContentEditable>
        </foreignObject>
    );
};
