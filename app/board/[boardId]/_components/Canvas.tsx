...
export const Canvas = ({ boardId}: CanvasProps) => {
    ...
    const insertLayer = useMutation(({ storage, setMyPresence }, layerType: LayerType.Ellipse | LayerType.Rectangle | LayerType.Text | LayerType.Note, position: Point) => {
        ...
        if (liveLayers.size >= MAX_LAYERS) {
            toast.error('Max Layers reached')
            return;
        }
    ...
    return (
        <main className="h-full w-full relative bg-neutral-100 touch-none">
            ...
            <Toolbar canvasState={canvasState} setCanvasState={setCanvasState} canRedo={canRedo} canUndo={canUndo} undo={history.undo} redo={history.redo}></Toolbar>
            <SelectionTools camera={camera} setLastUsedColor={setLastUsedColor}></SelectionTools>
                ...
                <g style={{ transform: `translate(${camera.x}px, ${camera.y}px)` }}>
                    ...
                    {canvasState.mode === CanvasMode.SelectionNet && canvasState.current != null && (
                        <rect className="fill-blue-500/5 stroke-blue-500 stroke-1"
                            ...
                            height={Math.abs(canvasState.origin.y - canvasState.current.y)}></rect>
                    )}
                    <CursorsPresence></CursorsPresence>
                    {pencilDraft != null && pencilDraft.length > 0 && (
                        <Path points={pencilDraft} fill={colorToCss(lastUsedColor)} x={0} y={0}></Path>
                    )}
                </g>
            </svg>
        </main>
